...
- **Gemini Reasoning Engine**: The application securely calls the Gemini API (`gemini-3-flash-preview`) to power the agent's reasoning loop and text embeddings.
...
1. The chunks are sent to the `gemini-embedding-2-preview` API to generate high-dimensional vectors.
